...
cosmo=FlatLambdaCDM(H0=h*100, Om0=Om, Tcmb0=Tcmb0)

# Coma redshift
z=0.0234
DA=cosmo.angular_diameter_distance(z).value*np.pi/180./60. # Mpc / arcmin

def calc_Pei(gamma, alpha, beta, Ytot, thetas):
  Ytot_int=(1./alpha)*Gammafun((-gamma+3.)/alpha)*Gammafun((beta-3.)/alpha)/Gammafun((-gamma+beta)/alpha)
  Pei=Ytot*mec2/(4.*np.pi*sigma_T*Ytot_int*thetas**3*DA*Mpc2m) # keV/m^3
  return Pei
...
if __name__=='__main__':
  ...
  Pei=calc_Pei(c, a, b, Ytot, theta_s)
  print('Pei=', Pei, 'keV/m^3')
